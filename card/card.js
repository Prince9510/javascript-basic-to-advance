...
function bet(response) {
  ...
  let findMatch = newDeck.findIndex(
    (ele) =>
      ele.number == obj.num &&
      ele.suit == obj.suit &&
  ...
  console.log(newDeck[findMatch]);

  if (response == "high") {
    if (
      Number(newDeck[findMatch].value) <= Number(newDeck[findMatch - 1].value)
    ) {
      console.log(newDeck[findMatch].value, newDeck[findMatch - 1].value);
